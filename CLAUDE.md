...
## 发版前必须跑一次 `pnpm test:engines`

日常 CI **不装图表引擎**（七个全装约 340MB，其中 d2 一个 91.4MB，而且有几个包要在机器上现编译，是最常装不上的那类）。所以真装真渲染的那批测试只在本地跑：

```bash
pnpm test:engines
```

**发版前必须跑一次**——「本地手动跑」少了这条就等于「永远不跑」。引擎没装时那些用例会打印一行跳过说明，跳过要看得见，否则它会被当成通过。
