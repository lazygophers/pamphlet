...
**图表引擎**:
把图源变成矢量图的那个东西。Mermaid 内置在主包里；其余七个各住在一个[引擎包]里。
_Avoid_: 渲染器, 适配器, backend

**引擎包**:
一个官方 npm 包，里面是某个图表引擎的适配代码（`@nekoleapuki/pamphlet-engine-*`）。用到哪个装哪个，不装的人一个字节都不多。
_Avoid_: 插件, 适配器包, engine plugin

**哨兵**:
渲染前当作主题色喂给引擎的一组怪颜色（`#ff0001`…`#ff0006`），渲染完在输出里换成 CSS 变量。「哪个颜色对应哪个位置」因此不靠人维护表格。
_Avoid_: 占位色, 魔法色, magic color

**外部命令引擎**:
[声明式引擎]的落地形态：Pamphlet 起一个外部进程，图源写进它的标准输入、SVG 从标准输出读回来。官方的 PlantUML 引擎走的就是这条。
_Avoid_: 子进程渲染器, CLI 引擎, shell 引擎
...
**结构化写法**:
用容器指令写图（`:::flow` 等十七种），指令体是「声明块 + 关系块」两段。区别于「图表围栏」里直接写图源的写法，两者永久并存。
_Avoid_: 自有语法, DSL, 简写, 别名

**声明块**:
结构化写法里列出「有哪些东西」的那一块（流程图的 `nodes:`、时序图的 `participants:`）。每一行是一个条目。
_Avoid_: 节点块, 定义段, declarations

**关系块**:
结构化写法里列出「它们之间怎么连」的那一块（流程图的 `edges:`、时序图的 `messages:`）。每一行是一个条目。
_Avoid_: 连线块, 边, relations

**块名**:
声明块或关系块开头那一行的名字，独占一行、以冒号结尾（`nodes:`）。**不是「标题」**——标题归 Markdown 的 `#`。
_Avoid_: 字段, key, 段落名, 标题

**条目**:
块名下面缩进写的那一行，一行一条。声明块的条目是一个声明，关系块的条目是一条关系。
_Avoid_: 项, 列表项, entry, 行
...
**用途说明**:
一套主题带的那一句话，说清它适合哪一类文档（`Theme.purpose`，中英各一句）。诊断 `DOC-106` 和文档站都从这里取。
_Avoid_: label, 标题, 描述
...
**声明式引擎**:
在 frontmatter 里用一条外部命令描述的自定义图表引擎。**面向第三方的唯一扩展点**——项目不向第三方提供 JS 层面的引擎接口（官方[引擎包]用的是内部抽象，不承担兼容承诺）。
