...
**手工发布建立的只是「包存在」，不建立「信任关系」**。七个引擎包手工发了 0.0.2 之后，CI 走 OIDC 仍然返回 404（`ERR_PNPM_AUTH_TOKEN_EXCHANGE ... status code 404`）；这个错误本身不足以证明具体哪个权限字段有误，需逐包检查配置。可信发布者是**逐包**配置的：在 npm 网站打开包的 Settings，填写 GitHub 组织、仓库、工作流文件名；官方步骤见 <https://docs.npmjs.com/trusted-publishers/>。本仓库新增包的路径是：手工发首个版本 → 在包设置里配可信发布者 → 后续未发布版本由 CI 发布。

**所有版本已存在时，工作流成功只证明跳过发布正常，不证明 OIDC 发布授权有效。** 验证信任关系必须查看配置，并在真正发布新版本时确认成功；不要为了验证而重复发布已有版本，也不要把空推送后的绿色状态当成发布授权的证据。
