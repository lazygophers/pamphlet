...
> **「三个」这个数字已被 [0048](./0048-engines-are-separate-packages.md) 改写**：图表引擎各自成包之后，仓库里是十一个包。本文的**判据**仍然有效——0048 正是照着它那句「真需要拆时往外拆是加法，不破坏使用方」做的，被否掉的十个包（`core` / `diagrams` / `html` / …）一个都没有回来。
