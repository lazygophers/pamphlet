...
亮暗两套变量同时存在于产物里，靠 CSS 的 `prefers-color-scheme` 切换 —— **没有一行 JavaScript 参与**。
...
**收窄之后的承诺是完全成立的**：CSP、`location.hash` 深链、data URI 字体、`prefers-color-scheme` 全部在 `file://` 下实测通过。留一个做不到的承诺，比收窄承诺伤害更大。
