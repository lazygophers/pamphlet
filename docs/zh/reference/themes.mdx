...
下面每一套有两张截图和一份**可以点开的完整样例**。两张分别是**屏幕上**（深色，产物的缺省，不看读者的系统设置）和**打印时**（浅色，唯一的例外）。三者出自同一次真实编译，源文档是同一份 `examples/demo.md` —— 所以看到的差别全部来自主题本身。

截图看不出 Tab 能不能点、折叠块怎么展开、侧边菜单跟不跟着滚，这些**要点进样例里试**。
