...
三部分：**表头一行、分隔行、若干数据行**。分隔行是必须的，没有它就不是表格；竖线两侧的空格随意，不需要对齐（对齐只是为了源文档好读）。
...
行内的东西都能写：**粗体**、`行内代码`、[链接](https://example.com)、图片。

**不能写的是块级的东西**：单元格里放不了代码块、列表、多个段落。真要放，把那部分挪到表格外面，或者改用[折叠块](/write/directives/collapse)。

内容里要写竖线 `|` 时，前面加反斜杠 `\|`，否则它会被当成分隔符。

### 没有合并单元格

GFM 表格**不支持跨行跨列合并**，也没有多层表头。需要合并的表格通常意味着它该拆成两张。

### 长相与窄屏

表格样式跟着主题走：边框用 `--pf-table-border`，表头底色用 `--pf-table-header-bg`（见[主题 token](/reference/theme-tokens)）。不同主题差别很大——`paper` 和 `architecture` 是只有上下粗线的三线表，`manual` 是表头吸顶加斑马纹。
